...
package classes.quietless.tools {
	...
	// sprite saved position //
		public var spriteX:Number=0;
		public var spriteY:Number=0;
		public var spriteWidth:Number;
		public var spriteHeight:Number;
		public var savedStatus:Boolean=false;
		//public var sObj:SharedObject;
		
		public var theData:transformData;
...
//--------------------------------------------------------------//			
		...
		{
			//sObj = SharedObject.getLocal("barclayCalibration");
		    _stroke = new Sprite(); 
 ...
 //- PUBLIC SETTERS ----------------------------------------------------------------------		
		...
		public function set targets($targets:Array):void
		{
			_targets = $targets;
			_target=$targets[0];
			/*if (sObj.data.x==undefined || sObj.data.y==undefined ) {
				trace ("Sheredobject undefined");
			} else {
				_target.x=Number(sObj.data.x);
				_target.y=Number(sObj.data.y);
				_target.width=Number(sObj.data.width);
				_target.height=Number(sObj.data.height);
			trace("Shared width:"+sObj.data.width.toString());
			trace("Shared height:"+sObj.data.height.toString());
			}*/
		}	
		...
		public function getCoords():transformData
		{
			//var theData=new transformData(_p1.x,_p1.y,_target.width,_target.height);
			/*sObj.data.x=_target.x;
			sObj.data.y=_target.y;
			sObj.data.width=_target.width;
			sObj.data.height=_target.height;
			sObj.flush();*/
 ...
 //- BOUNDING BOX & HANDLE DRAW METHODS ----------------------------------------------------------------------					
		...
		// reposition handles //
			...
			var p4 = _stroke.localToGlobal(new Point(_stroke_br.x, _stroke_br.y));
				...
				theData=new transformData(tl.x,tl.y,_target.width,_target.height);
		}		
